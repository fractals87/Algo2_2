...
public class AdjListUndirWeight implements WeightedGraph{
	...
	public WeightedGraph getPrimMST(int startingVertex) throws UnsupportedOperationException, IllegalArgumentException {
		...
		VisitForest forest=new VisitForest(this, VisitType.DFS);
		PriorityQueueArr q = new PriorityQueueArr(this.size());
		AdjListUndirWeight res = new AdjListUndirWeight();
		for(int i=0; i< this.size(); i++)
			res.addVertex();
		
		q.modify_priority(startingVertex, 0);
		int indexVertex = q.dequeue();
		while(indexVertex!=-1) {
			//res.addVertex();
			if(indexVertex != startingVertex) {
				int parent = forest.getPartent(indexVertex);
				res.addEdge(parent, indexVertex);
				res.setEdgeWeight(parent, indexVertex, this.getEdgeWeight(parent, indexVertex));
			}
			forest.setColor(indexVertex, Color.BLACK );
			for(Edge v : vertexList.get(indexVertex)) {
				//q.modify_priority(v.vertex, (int)v.weight);
				if(forest.getColor(v.vertex) != Color.BLACK && (q.queue[v.vertex] > v.weight || q.queue[v.vertex] == -1)  ) {
					forest.setParent(v.vertex, indexVertex);
					forest.setDistance(v.vertex, v.weight);
					q.queue[v.vertex] = (int)v.weight;
				}
			}
			indexVertex = q.dequeue();
		}
		return res;
	}
